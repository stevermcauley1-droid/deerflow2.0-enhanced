...
def _detect_mode_with_config(config_content: str) -> str:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        output = _run_detect_command(command)

        return output


def _run_detect_command(command: str) -> str:
    ...
    try:
        ...
        stdout = (exc.output or "").replace("\x00", "").lower()
        if "the service cannot be started" in stdout or "bash/0x80070422" in stdout:
            pytest.skip("bash/WSL service is unavailable in this environment")
        raise
